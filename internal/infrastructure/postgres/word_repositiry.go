...
func (r *wordRepository) GetIng(ctx context.Context, query string, page_number int) ([]*word.Word, int, error) {
	offset := (page_number - 1) * LIMIT
	searchQuery := strings.TrimSpace(query)
	if searchQuery == "" {
		return []*word.Word{}, 0, nil
	}
	...
	rows, err := r.db.QueryContext(ctx, sql, "%"+searchQuery+"%", searchQuery, searchQuery+"%", LIMIT, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var w word.Word
		var count int
		if err := rows.Scan(&w.WordID, &w.Word, &w.Translation, &w.SpeechPart, &w.Topic, &w.DictID, &count); err != nil {
			return nil, 0, err
		}
		if quantity == 0 {
			quantity = count
	...
	return results, quantity, rows.Err()
}

func (r *wordRepository) GetIngFiltered(ctx context.Context, query string, tg_user_id int64, page_number int) ([]*word.Word, int, error) {
	offset := (page_number - 1) * LIMIT
	...
	rows, err := r.db.QueryContext(ctx, sql, "%"+searchQuery+"%", searchQuery, searchQuery+"%", tg_user_id, LIMIT, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		var w word.Word
		if err := rows.Scan(&w.WordID, &w.Word, &w.Translation, &w.SpeechPart, &w.Topic, &w.DictID, &w.DictAbbr, &quantity); err != nil {
			return nil, 0, err
		}
		results = append(results, &w)
	}
...
func (r *wordRepository) GetRus(ctx context.Context, query string, page_number int) ([]*word.Word, int, error) {
	offset := (page_number - 1) * LIMIT
	searchQuery := strings.TrimSpace(query)
	if searchQuery == "" {
		return []*word.Word{}, 0, nil
	...
	for rows.Next() {
		var w word.Word
		if err := rows.Scan(&w.WordID, &w.Word, &w.Translation, &w.SpeechPart, &w.Topic, &w.DictID, &quantity); err != nil {
			return nil, 0, err
		}
		results = append(results, &w)
	}
...
func (r *wordRepository) GetRusFiltered(ctx context.Context, query string, tg_user_id int64, page_number int) ([]*word.Word, int, error) {
	offset := (page_number - 1) * LIMIT
